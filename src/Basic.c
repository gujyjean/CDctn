...
SEXP datGenC(SEXP node,SEXP maxdeg,SEXP ordex,SEXP ts,SEXP noi,SEXP nobs,SEXP ivn,SEXP fX,SEXP beta)
{
    ...
    PROTECT(data=allocVector(REALSXP, xnode*xnobs*xnoi));
    rdata=REAL(data);
    ++nProtected;
    ...
    for (k=0;k<xnoi;k++)
    {
        ...
        for (i=0;i<xnobs;i++)
		{
			...
			for (j=0;j<xnode;j++)
			{
				...
				{
					rdata[tmp2+tmp3+jx-1]=rnorm(0.0,1.0);
				}
				else
				{
					pp=0.0;
					...
					rdata[tmp2+tmp3+jx-1]=pp+rnorm(0.0,1.0);
				}
			}
		}
    }
...
SEXP prodG(SEXP xnode,SEXP xlist,SEXP ylist,SEXP obslength)
{
    ...
    ++nProtected;
    SET_STRING_ELT(names,0,mkChar("inprod"));
    SET_STRING_ELT(names,1,mkChar("fprod"));
    setAttrib(retList,R_NamesSymbol,names);

    //creat the elements of the return list
    SEXP inprod;
    PROTECT(inprod=allocVector(REALSXP,node2));
    inprod_v=REAL(inprod);
    ++nProtected;
    SEXP fprod;
    PROTECT(fprod=allocVector(REALSXP,node3));
    fprod_v=REAL(fprod);
    ++nProtected;
